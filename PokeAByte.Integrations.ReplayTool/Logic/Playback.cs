...
public sealed class Playback
{
    ...
    public byte[] GetReconstructedState(int key)
    {
        ...
        var saveState = _playbackStates[key];
    ...
    /*public byte[] GetReconstructedState(int key)
    {
        ...
        //find the frame we want
        var saveState = _playbackStates[key];/*_playbackStates.FirstOrDefault(p => p.Frame == frame);#1#
    ...
    private byte[] ReconstructToFrame(int frame, Keyframe startState)
    {
        ...
        foreach (var saveState in _playbackStates
                     .Where(playbackState => playbackState.Frame > startState.Frame && playbackState.Frame <= frame))
        {
            if (saveState.IsKeyframe)
            {
                //this shouldn't occur, but if it does, then we already have the reconstructed state, use that
                lastState = saveState.SaveState;
            }
            else
            {
                var decompressedDelta = ZStdHelpers.Decompress(saveState.SaveState);
                if (decompressedDelta.Length == 0)
                {
                    //todo: handle this
                    return [];
                }
                var reconstructedState = Fossil.Delta.Apply(lastState, decompressedDelta);
